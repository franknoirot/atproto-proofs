...
```
pnpm present                      # capture a run into presentation/data.js
open presentation/index.html      # 24 slides, no server needed
```

An interactive deck that walks the demo, visualises who owns what, and ends on
how to adapt the framework to another regulated domain. Arrow keys or space to
step; `n` for presenter notes, `t` for the table view behind each chart, `o` for
an overview, `?` for keys. `#12` deep-links to a slide, `#12.2` to a slide with
two build steps revealed.

Every CID, verdict, axiom set, swing curve and generated Lean module in it was
captured from a real run — a deck arguing "verdicts are computations, not
assertions" should not quote invented hashes. The swing curves are computed by
the Lean theory itself rather than re-derived for the chart.
...
| `presentation/` | the deck; `data.js` is generated by `pnpm present` |
